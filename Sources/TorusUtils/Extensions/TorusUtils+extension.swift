...
extension TorusUtils {
    ...
    func combinations<T>(elements: ArraySlice<T>, k: Int) -> [[T]] {
        ...
        let head = [first]
        let subcombos = self.combinations(elements: elements.dropFirst(), k: k - 1)
        var ret = subcombos.map {
            head + $0
        }
        ret += self.combinations(elements: elements.dropFirst(), k: k)

        return ret
    }

    func combinations<T>(elements: [T], k: Int) -> [[T]] {
        return self.combinations(elements: ArraySlice(elements), k: k)
    }
    ...
    func retrieveDecryptAndReconstruct(
        endpoints: [String], extraParams: Data, verifier: String, tokenCommitment: String,
        nodeSignatures: [[String: String]], verifierId: String, lookupPubkeyX: String,
        lookupPubkeyY: String, privateKey: String) -> Promise<(String, String, String)>
    {
        // Rebuild extraParams
        var rpcdata = Data()
        do {
            if let loadedStrings = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(
                extraParams)
                as? [String: Any]
            {
                let value =
                    [
                        "verifieridentifier": verifier, "verifier_id": verifierId,
                        "nodesignatures": nodeSignatures, "idtoken": tokenCommitment,
                    ] as [String: Any]
                let keepingCurrent = loadedStrings.merging(value) { current, _ in
                    current
                }
        ...
        var requestPromises = [Promise<(data: Data, response: URLResponse)>]()
        for el in endpoints {
        ...
        var globalCount = 0
        var shareResponses = Array<[String: String]?>.init(
            repeating: nil, count: requestPromises.count)
        var resultArray = [Int: [String: String]]()
        var errorStack = [Error]()
        for (i, rq) in requestPromises.enumerated() {
            rq.then { data, _ -> Promise<[Int: String]> in
                self.logger.info(
                    "retrieveDecryptAndReconstuct:", String(decoding: data, as: UTF8.self))
                ...
                let lookupShares = shareResponses.filter {
                    $0 != nil
                } // Nonnil elements

                // Comparing dictionaries, so the order of keys doesn't matter
                let keyResult = self.thresholdSame(
                    arr: lookupShares.map {
                        $0
                    }, threshold: Int(endpoints.count / 2) + 1) // Check if threshold is satisfied
                ...
                self.logger.trace(
                    "retrieveDecryptAndReconstuct - data after decryptIndividualShares:", data)
                let filteredData = data.filter {
                    $0.value != TorusError.decodingFailed.debugDescription
                }
                if filteredData.count < Int(endpoints.count / 2) + 1 {
                    throw TorusError.thresholdError
                }
                return self.thresholdLagrangeInterpolation(
                    data: filteredData, endpoints: endpoints, lookupPubkeyX: lookupPubkeyX,
                    lookupPubkeyY: lookupPubkeyY)
            }.done { x, y, z in
                seal.fulfill((x, y, z))
            }.catch { err in
                ...
                if nsErr.code == -1003 {
                    ...
                    self.logger.error(
                        "retrieveDecryptAndReconstuct: DNS lookup failed, node (\(userInfo["NSErrorFailingURLKey"] ?? "")) is probably offline."
                    )
    ...
    func commitmentRequest(
        endpoints: [String], verifier: String, pubKeyX: String, pubKeyY: String, timestamp: String,
        tokenCommitment: String) -> Promise<[[String: String]]>
    {
        ...
        var resultArrayObjects = Array<JSONRPCresponse?>.init(
            repeating: nil, count: requestPromises.count)
        var lookupCount = 0
        var globalCount = 0
        for (i, rq) in requestPromises.enumerated() {
            rq.done { data, _ in
                ...
                let lookupShares = resultArrayStrings.filter {
                    $0 as? String != nil
                } // Nonnil elements
                if lookupShares.count >= Int(endpoints.count / 4) * 3 + 1, !promise.isFulfilled {
                    let nodeSignatures = resultArrayObjects.compactMap {
                        $0
                    }.map {
                        $0.result as! [String: String]
                    }
                ...
                if nsErr.code == -1003 {
                    // In case node is offline
                    self.logger.error(
                        "commitmentRequest: DNS lookup failed, node (\(userInfo["NSErrorFailingURLKey"] ?? "")) is probably offline."
                    )
    ...
    func retrieveIndividualNodeShare(
        endpoints: [String], extraParams: Data, verifier: String, tokenCommitment: String,
        nodeSignatures: [[String: String]], verifierId: String) -> Promise<[Int: [String: String]]>
    {
        // Rebuild extraParams
        var rpcdata = Data()
        do {
                ...
                as? [String: Any]
            {
                ...
                let keepingCurrent = loadedStrings.merging(newValue) { current, _ in
                    current
                }
        ...
        for (i, rq) in requestPromises.enumerated() {
            rq.done { data, _ in
                self.logger.info(
                    "retreiveIndividualNodeShares:", String(decoding: data, as: UTF8.self))
                let decoded = try JSONDecoder().decode(JSONRPCresponse.self, from: data)
                if decoded.error != nil {
                ...
                if nsErr.code == -1003 {
                    // In case node is offline
                    self.logger.error(
                        "retreiveIndividualNodeShares: DNS lookup failed, node (\(userInfo["NSErrorFailingURLKey"] ?? "")) is probably offline."
                    )
                } else {
    ...
    func decryptIndividualShares(shares: [Int: [String: String]], privateKey: String) -> Promise<
        [Int: String]
    > {
        ...
        for (_, el) in shares.enumerated() {
            ...
            let ephemPubKey = secp256k1_pubkey.init(data: self.array32toTuple(Array(ephermOne)))

            guard
                // Calculate g^a^b, i.e., Shared Key
                let data = Data(hexString: privateKey),
                let sharedSecret = self.ecdh(
                    pubKey: ephemPubKey, privateKey: data),
                let sharedSecretData = sharedSecret.data,
                let sharedSecretPrefix = self.tupleToArray(sharedSecretData).prefix(32),
            ...
            do {
                // AES-CBCblock-256
                let aes = try AES(
                    key: AesEncryptionKey.hexa, blockMode: CBC(iv: iv), padding: .pkcs7)
    ...
    func thresholdLagrangeInterpolation(
        data filteredData: [Int: String], endpoints: [String], lookupPubkeyX: String,
        lookupPubkeyY: String) -> Promise<(String, String, String)>
    {
        let (tempPromise, seal) = Promise<(String, String, String)>.pending()
        // all possible combinations of share indexes to interpolate
        let shareCombinations = self.combinations(
            elements: Array(filteredData.keys), k: Int(endpoints.count / 2) + 1)
        var totalInterpolations = 0
        for shareIndexSet in shareCombinations {
            var sharesToInterpolate: [Int: String] = [:]
            shareIndexSet.forEach {
                sharesToInterpolate[$0] = filteredData[$0]
            }
            self.lagrangeInterpolation(shares: sharesToInterpolate).done { data -> Void in
                // Split key in 2 parts, X and Y
                let finalPrivateKey = data.web3.hexData!
                guard
                    let publicKey = SECP256K1.privateToPublic(privateKey: finalPrivateKey)?.subdata(
                        in: 1..<65)
                else {
                    seal.reject(TorusError.decodingFailed)
                    return
                ...
                self.logger.trace(
                    "retrieveDecryptAndReconstuct: private key rebuild", data, pubKeyX as Any,
                    pubKeyY as Any)
    ...
    func lagrangeInterpolation(shares: [Int: String]) -> Promise<String> {
        ...
        _ = shares.map {
            shareList[BigInt($0.key + 1)] = BigInt($0.value, radix: 16)
        }
    ...
    public func keyLookup(endpoints: [String], verifier: String, verifierId: String) -> Promise<
        [String: String]
    > {
        ...
        URLSession.shared.dataTask(.promise, with: allowHostRequest).done { _ in
            // swallow
        }.catch { error in
        ...
        var promisesArray = [Promise<(data: Data, response: URLResponse)>]()
        for el in endpoints {
            let rq = self.makeUrlRequest(url: el)
        ...
        for (i, pr) in promisesArray.enumerated() {
            pr.done { data, _ in
                ...
                let lookupShares = resultArray.filter {
                    $0 != nil
                } // Nonnil elements
                let keyResult = self.thresholdSame(
                    arr: lookupShares, threshold: Int(endpoints.count / 2) + 1) // Check if threshold is satisfied
                ...
                if tmpError.code == -1003 {
                    // In case node is offline
                    self.logger.error(
                        "keyLookup: DNS lookup failed. Node (\(userInfo["NSErrorFailingURLKey"] ?? "")) is probably offline"
                    )
    ...
    public func keyAssign(
        endpoints: [String], torusNodePubs: [TorusNodePub], verifier: String, verifierId: String) -> Promise<JSONRPCresponse>
    {
        ...
        for (i, endpoint) in newEndpoints.enumerated() {
            serialQueue.async {
                ...
                let SignerObject = JSONRPCrequest(
                    method: "KeyAssign", params: ["verifier": verifier, "verifier_id": verifierId])
                let rpcdata = try! encoder.encode(SignerObject)
                var request = self.makeUrlRequest(url: "https://signer.tor.us/api/sign")
                request.addValue(
                    torusNodePubs[index].getX().lowercased(), forHTTPHeaderField: "pubKeyX")
                request.addValue(
                    torusNodePubs[index].getY().lowercased(), forHTTPHeaderField: "pubKeyY")
                self.logger.trace(
                    "KeyAssign: nodekeys: ", torusNodePubs[index].getX().lowercased(),
                    torusNodePubs[index].getY().lowercased())
                self.logger.trace(
                    "KeyAssign: requestToSigner: ", String(data: rpcdata, encoding: .utf8) as Any)

                firstly {
                    URLSession.shared.uploadTask(.promise, with: request, from: rpcdata)
                }.then { data, _ -> Promise<(data: Data, response: URLResponse)> in
                    self.logger.trace(
                        "KeyAssign: responseFromSigner: ", String(decoding: data, as: UTF8.self))

                    let decodedSignerResponse = try JSONDecoder().decode(
                        SignerResponse.self, from: data)
                    let keyassignRequest = KeyAssignRequest(
                        params: ["verifier": verifier, "verifier_id": verifierId],
                        signerResponse: decodedSignerResponse)
    ...
    func array32toTuple(_ arr: [UInt8]) -> (
    ...
    {
        return (
            arr[0] as UInt8, arr[1] as UInt8, arr[2] as UInt8, arr[3] as UInt8, arr[4] as UInt8,
            arr[5] as UInt8, arr[6] as UInt8, arr[7] as UInt8, arr[8] as UInt8, arr[9] as UInt8,
            arr[10] as UInt8, arr[11] as UInt8, arr[12] as UInt8, arr[13] as UInt8,
            arr[14] as UInt8,
            arr[15] as UInt8, arr[16] as UInt8, arr[17] as UInt8, arr[18] as UInt8,
            arr[19] as UInt8,
            arr[20] as UInt8, arr[21] as UInt8, arr[22] as UInt8, arr[23] as UInt8,
            arr[24] as UInt8,
            arr[25] as UInt8, arr[26] as UInt8, arr[27] as UInt8, arr[28] as UInt8,
            arr[29] as UInt8,
            arr[30] as UInt8, arr[31] as UInt8, arr[32] as UInt8, arr[33] as UInt8,
            arr[34] as UInt8,
            arr[35] as UInt8, arr[36] as UInt8, arr[37] as UInt8, arr[38] as UInt8,
            arr[39] as UInt8,
            arr[40] as UInt8, arr[41] as UInt8, arr[42] as UInt8, arr[43] as UInt8,
            arr[44] as UInt8,
            arr[45] as UInt8, arr[46] as UInt8, arr[47] as UInt8, arr[48] as UInt8,
            arr[49] as UInt8,
            arr[50] as UInt8, arr[51] as UInt8, arr[52] as UInt8, arr[53] as UInt8,
            arr[54] as UInt8,
            arr[55] as UInt8, arr[56] as UInt8, arr[57] as UInt8, arr[58] as UInt8,
            arr[59] as UInt8,
            arr[60] as UInt8, arr[61] as UInt8, arr[62] as UInt8, arr[63] as UInt8)
    }
}
